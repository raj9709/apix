...
# Configuration
# ─────────────────────────────────────────────────────────────
SERPAPI_KEY = "b4e15b120c3d5a96cb7bafe404f9bce3b89015d80690d1a7f682f815cbda6639"
...
# ─────────────────────────────────────────────────────────────
# LIVE SERPAPI INGESTION ENDPOINT (ALL HORIZONS)
# ─────────────────────────────────────────────────────────────
@app.post("/api/admin/run-ingestion")
async def run_ingestion():
    if not SERPAPI_KEY or SERPAPI_KEY == "PASTE_YOUR_SERPAPI_KEY_HERE":
        raise HTTPException(status_code=400, detail="SerpAPI Key missing in backend!")

    # Define all required SIH tracking windows
    ...
    try:
        async with httpx.AsyncClient() as client:
            # Loop through all 5 advance purchase horizons
            for window, days in horizon_days.items():
                target_date = date.today() + timedelta(days=days)
                url = f"https://serpapi.com/search.json?engine=google_flights&departure_id=DEL&arrival_id=BOM&outbound_date={target_date}&type=2&currency=INR&hl=en&api_key={SERPAPI_KEY}"
                ...
                # Grab top 3 cheapest flights per horizon (15 total) to keep the demo clean
        ...
        if not new_fares:
            raise ValueError("SerpApi returned no flights for any horizon.")

        # Inject new data at the top of the Transparency Grid
        ...
        return {
            ...
            "message": f"Live data extracted! Added {len(new_fares)} fresh flights across all horizons."
        }

    except Exception as e:
        print(f"Live Scraping Failed: {e}")
        # Silent fallback ensures the demo never crashes on stage
        load_and_process_dataset()
